...
from modules.emo2vec.modules import AltBlock
from modules.emo2vec.audio import AudioEncoder
...
class Emotion2vec(torch.nn.Module):
    ...
    def forward(
        ...
        padding_mask=None,
        mask=True,
        features_only=True, # False originally
    ...
    def inference(
        ...
        # granularity = kwargs.get("granularity", "utterance") # utterance only
        ...
        features = self.extract_features(audio, padding_mask=None)

        #x = features["x"]
        embeddings = features["x"].squeeze(0)
        features = features["layer_results"] if extract_embedding else None
        ...
        for i, wav in enumerate(audio_sample_list):
            ...
            features = self.extract_features(source, padding_mask=None)
            x = features["x"]
            features = features["x"].squeeze(0).cpu().numpy()
            ...
            if output_dir and extract_embedding:
                np.save(os.path.join(output_dir, "{}.npy".format(key[i])), features)
    ...
    def load_weights(self, weights_path:str):
        ...
        for key in state_dict["model"].keys():
            if key == "proj.weight" or key == "proj.bias":
                continue
            new_key = re.sub(r'^d2v_model\.modality_encoders\.AUDIO', 'feature_extractor', key)
            new_key = re.sub(r'^d2v_model.', '', new_key)
